...
<head>
    ...
    <title>FinTask - Task Details</title>
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                @if($task->image)
                    <div class="mb-4 flex justify-center">
                        <img src="{{ asset('storage/' . $task->image) }}" alt="Task Image" class="rounded border">
                    </div>
                @endif
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-gray-900">{{ $task->due_date ? \Carbon\Carbon::parse($task->due_date)->format('M d, Y') : 'N/A' }}</p>
                </div>
                <div class="mb-4">
                ...
                <div class="flex items-center justify-between mt-6">
                    <a href="{{ route('tasks.edit', $task->id) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Edit Task</a>
                    <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" class="inline-block">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" onclick="return confirm('Are you sure you want to delete this task?')">Delete Task</button>
                    </form>
                    <a href="{{ route('tasks.index') }}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Back to Tasks</a>
                </div>
            </div>
        </div>
    </div>
