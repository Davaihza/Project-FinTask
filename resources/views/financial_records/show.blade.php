...
<head>
    ...
    <title>FinTask - Financial Record Details</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-8 main-content overflow-y-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-gray-800">Financial Record Details</h1>
            </div>

            <div class="bg-white p-6 rounded-lg shadow">
                @if($financialRecord->image)
                    <div class="mb-4 flex justify-center">
                        <img src="{{ asset('storage/' . $financialRecord->image) }}" alt="Record Image" class="rounded border">
                    </div>
                @endif
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Description:</label>
                    <p class="text-gray-900">{{ $financialRecord->description ?? 'N/A' }}</p>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Amount:</label>
                    <p class="text-gray-900">Rp{{ number_format($financialRecord->amount, 0, ',', '.') }}</p>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-gray-900">{{ \Carbon\Carbon::parse($financialRecord->date)->format('M d, Y') }}</p>
                </div>
                <div class="flex items-center justify-between mt-6">
                    <a href="{{ route('financial-records.edit', $financialRecord->id) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Edit Record</a>
                    <form action="{{ route('financial-records.destroy', $financialRecord->id) }}" method="POST" class="inline-block">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" onclick="return confirm('Are you sure you want to delete this record?')">Delete Record</button>
                    </form>
                    <a href="{{ route('financial-records.index') }}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Back to Financial Records</a>
                </div>
            </div>
        </div>
    </div>
